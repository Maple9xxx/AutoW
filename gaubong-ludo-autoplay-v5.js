...
// @version      4.0.0
// @description  Auto Ludo PRO v4. Fix canFinish overshoot, No-reload loop, Auto-next game, HUD v2 draggable.
// @author       AutoPlay Pro
...
(function() {
    ...
    //
    // 🎯 TRIẾT LÝ v4: Sửa gốc, không vá ngọn. Hiệu suất không đánh đổi tính đúng đắn.
    // ================================================================
    ...
    function chonQuanPro(pieces, myPos, dice, parsed) {
        ...
        // GitHub Bot constants (safety=10, killBonus=300, base=100)
        const safety = 10;
        const killBonus = 300;
        const baseDistanceBonus = safety * 10; // 100

        // hasTokensHome: còn quân nào trong chuồng không?
        const hasTokensHome = Object.values(vitri).some(coord => !coord);

        // Build piece info
        const info = pieces.map(p => pieceInfo(myPos, p, vitri));

        log(`[GitHub AI] Chon quan: xx=${dice}, co ${info.length} quan`);
        for (const i of info) {
            log(`   Q${i.pieceId}: pathPos=${i.pathPos} adv=${i.adv} inHS=${i.inHS} d2f=${i.d2f} safe=${i.safe}`);
        }

        // -----------------------------------------------------------------
        // SCORING LOOP - GitHub Bot.pickToken() logic, adapted for gaubong:
        // -----------------------------------------------------------------
        let scored = info.map(pi => {
            let score = 0;
            const reasons = [];

            // sqSixBonus: GitHub dung (s * safety/10) de scale penalty
            // theo vi tri quan (quan cang xa chuong = penalty cang nang)
            const sqSixBonus = pi.pathPos >= 0 ? (pi.pathPos * safety / 10) : 0;

            // == OVERRIDE: canFinish => Uu tien tuyet doi ==
            // (gaubong.us khong bounce - d2f phai === dice moi finish duoc)
            // GitHub khong can check nay vi walkSteps() tu xu ly,
            // nhung o gaubong.us API phai goi dung quan dung so.
            const canFinish = !pi.atHome && pi.d2f === dice;
            if (canFinish) {
                score = 10000;
                reasons.push('🏁 ve dich! (override)');
                return { ...pi, score, reasons };
            }

            // == GITHUB STEP 1: previousDs - Dem dich dang de doa quan nay ==
            // (tokensInRange truoc khi di chuyen)
            // Dich nam phia sau trong tam 1-6 buoc -> co the capture minh
            // -> Di chuyen khoi day = thoat hiem = cong diem
            if (!pi.atHome && !pi.inHS && !pi.safe) {
                for (const e of enemies) {
                    if (e.safe) continue;
                    const dist = forwardDist(e.pathPos, pi.pathPos);
                    if (dist >= 1 && dist <= 6) {
                        score += baseDistanceBonus;           // +100
                        score += (7 - dist) * safety;         // +60 den +10
                        reasons.push(`🚨 thoat hiem dist=${dist}(+${baseDistanceBonus + (7-dist)*safety})`);
                    }
                }
            }

            // == GITHUB STEP 2: Vacate entrance square (s===5 -> sq6) ==
            // Neu dang dung ngay o entrance cua minh VA con quan trong chuong
            // -> Nhuong cho de ra them quan
            if (!pi.atHome && !pi.inHS) {
                if (pi.pathPos === EN[myPos] && hasTokensHome) {
                    score += safety; // +10
                    reasons.push('↗️ nhuong entrance (+10)');
                }
            }

            // == GITHUB STEP 3: Exit home (steps===-1, dice===6) ==
            // Quan trong chuong + ra duoc -> diem cao (tuong duong +300)
            if (pi.atHome && dice === 6) {
                score += baseDistanceBonus * 3; // +300
                reasons.push('🏠 ra quan (+300)');
            }

            // == GITHUB STEP 4: About to enter home stretch ==
            // (s <= 52 && s+d > 52 -> sap vao HS)
            // Map: adv + dice >= PL (51) -> tran vao home stretch
            if (!pi.atHome && !pi.inHS && pi.adv >= 0) {
                if (pi.adv + dice >= PL) {
                    score += baseDistanceBonus; // +100
                    reasons.push('🏁 sap vao HS (+100)');
                }
            }

            // == GITHUB STEP 5: Inside home stretch (s > 52) ==
            // RESET score ve am: quan trong HS da an toan -> uu tien thap hon
            // depth=1 (moi vao) -> score=-9
            // depth=6 (gan dich) -> score=-59
            // Formula: score = depth * (-safety) + 1
            if (pi.inHS) {
                const depth = pi.hsIdx + 1;
                score = (depth * -safety) + 1; // RESET: -9 den -59
                reasons.push(`🏠 HS buoc${depth}(score=${score})`);
            }

            // == GITHUB STEP 6: Current pos = entrance cua doi thu (pre-move) ==
            // Neu dang dung o entrance cua doi thu VA ho con quan trong chuong
            // -> Nguy hiem: ho roll 6 se da minh ra
            // Note: trong gaubong.us entrance stars la safe cells -> !pi.safe chan
            if (!pi.atHome && !pi.inHS && !pi.safe && pi.pathPos >= 0) {
                for (const [opos, odata] of Object.entries(opponents)) {
                    const oppPos = parseInt(opos);
                    const oppEntrance = EN[oppPos];
                    const oppHasHome = Object.values(odata.vitri).some(c => !c);
                    if (pi.pathPos === oppEntrance && oppHasHome) {
                        score -= (baseDistanceBonus + sqSixBonus);
                        reasons.push(`⚠️ dung cua P${oppPos}(-${Math.round(baseDistanceBonus + sqSixBonus)})`);
                    }
                }
            }

            // == GITHUB STEPS 7-9: Simulate move -> Evaluate new position ==
            if (!pi.atHome && !pi.inHS && pi.pathPos >= 0) {
                const newPathPos = (pi.pathPos + dice) % PL;

                // -- STEP 7: Capture bonus (checkKill equivalent) --
                // Neu di chuyen den o co quan dich -> da dich ve chuong
                // Score = victimStepsWalked + victimSteps/10 + killBonus
                ...
                if (captureTarget) {
                    ...
                    score += victimTotalAdv / 10;          // stepsWalked proxy
                    score += captureTarget.pathPos / 10;   // victim.steps proxy
                    score += 4;                            // base bonus
                    score += killBonus;                    // +300
                    reasons.push(`⚔️ capture P${oppPos}(+${Math.round(killBonus + captureTarget.pathPos/10 + 4 + victimTotalAdv/10)})`);
                }

                // -- STEP 8: New pos = entrance cua doi thu (post-move) --
                // Bo qua neu landing square la safe cell (khong bi capture)
                if (!isSafePos(newPathPos)) {
                    for (const [opos, odata] of Object.entries(opponents)) {
                        const oppPos = parseInt(opos);
                        const oppEntrance = EN[oppPos];
                        const oppHasHome = Object.values(odata.vitri).some(c => !c);
                        if (newPathPos === oppEntrance && oppHasHome) {
                            score -= (baseDistanceBonus + sqSixBonus);
                            reasons.push(`⚠️ di vao cua P${oppPos}(-${Math.round(baseDistanceBonus + sqSixBonus)})`);
                        }
                    }
                }

                // -- STEP 9: nextDs - Dich de doa tai VI TRI MOI (post-move) --
                // (tokensInRange sau khi di chuyen)
                // Dich trong tam 1-6 buoc phia sau vi tri moi -> tru diem
                // KHONG ap dung neu landing = safe cell
                // LOAI TRU: blocking voi 2 quan (mini game khong cho phep)
                if (!isSafePos(newPathPos)) {
                    for (const e of enemies) {
                        if (e.safe) continue;
                        ...
                        if (dist >= 1 && dist <= 6) {
                            score -= baseDistanceBonus;            // -100
                            score -= (7 - dist) * safety;          // -60 den -10
                            reasons.push(`👁️ dich rinh vi tri moi dist=${dist}(-${baseDistanceBonus + (7-dist)*safety})`);
                        }
                    }
                }
            }
        ...
        // Sort: score cao nhat -> uu tien nhat
        // Tie-breaker: quan tien duoc xa hon tren ban duoc uu tien
        scored.sort((a, b) => {
        ...
        log(`[GitHub AI] Chon Q${best.pieceId} (score=${best.score}): ${best.reasons.slice(0,3).join(' | ')}`);
        ST.lastReason = `[GH] ${best.reasons.slice(0,2).join(' | ')}`;
        return best.pieceId;
    }
    ...
    async function tick() {
        ...
        // [v4] Stale state guard: nếu API liên tiếp trả về state không đổi → skip (tránh spam)
        // Hash đơn giản: round + status + dice value
    ...
    function mkUI() {
        ...
        p.innerHTML = `
            <div id="gb-drag-handle" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid #21262d;cursor:grab;">
                <span style="font-weight:700;font-size:15px;color:#58a6ff;">🎲 Ludo PRO <span style="font-size:10px;color:#484f58;">v4</span></span>
    ...
    // ================================================================
    function boot() {
        log('🎲 Ludo Auto-Play PRO v4.0 - canFinish Fix | No-Reload | Auto-Next | HUD v2');

...
// @version      9.1.0
// @description  Auto Ludo PRO v9.1. Depth-3 Expectiminimax + Dice6 BonusTurn + ThreatAware evalState
// @author       AutoPlay Pro
...
(function() {
    ...
    // v9.1.0- 27/06 - BUG FIX × 3 + NEAR-FINISH MEGA-BONUS:
    //   🐛 FIX #1 CRITICAL: expectiminimax không model dice=6 bonus turn
    //      ROOT CAUSE: d=6 gọi expectiminimax(ns, depth-1, false) — chuyển lượt cho địch
    //      luôn, thay vì giữ lượt cho mình (bonus roll). Sai ~16.7% expected value.
    //      FIX: d===6 → isOurTurn=true (vẫn lượt ta); d===6 (địch) → isOurTurn=false
    //   🐛 FIX #2 CRITICAL: Hard Rule 2 quá cứng — deploy khi quân gần về đích (d2f<=8)
    //      ROOT CAUSE: Đổ 6 + không kill → PHẢI deploy, kể cả khi quân cách đích 7 bước
    //      FIX: Xóa Hard Rule 2 hoàn toàn. Thay bằng near-finish mega-bonus tự nhiên.
    //   🐛 FIX #3 MODERATE: evalState thiếu Threat Awareness — không phạt quân đang bị kẹp
    //      FIX: Thêm danger penalty (-28/quân) cho quân ta bị địch kẹp trong 1-6 bước
    //      + opportunity bonus (+18/quân) cho quân địch ta đang trong tầm bắt
    //   🎯 NEAR-FINISH MEGA-BONUS: immScore thưởng thêm 120-360pt khi d2f<=6 (tuyến tính)
    //      Đảm bảo scoring tự nhiên ưu tiên quân sắp về hơn xuất quân mới (không cần rule)
    ...
    // v9.1: Thêm Threat Awareness — phạt quân ta bị kẹp + thưởng tầm bắt địch
    // ================================================================
    function evalState(state) {
        const _myPos = state.myPos;
        let score = 0;

        // Pre-collect my pieces' pathPos for opportunity bonus check
        const myActivePP = [];
        for (const coord of Object.values(state.pieces[_myPos] || {})) {
            if (!coord || coord === 'finished') continue;
            const pp = posFromCoord(coord);
            if (pp >= 0) myActivePP.push(pp);
        }
            ...
            for (const coord of Object.values(pcs)) {
                ...
                if (hsArr && hsArr.includes(coord)) {
                    ...
                    if (isMe) {
                        ...
                        // --- THREAT AWARENESS (v9.1): phạt khi bị địch kẹp ---
                        // Kiểm tra có quân địch nào đang trong tầm capture ta không
                        // (địch cách ta 1-6 bước phía sau = có thể ăn quân ta)
                        if (!isSafePos(pp)) {
                            for (const [ep, epcs] of Object.entries(state.pieces)) {
                                if (parseInt(ep) === _myPos) continue;
                                for (const ec of Object.values(epcs)) {
                                    if (!ec || ec === 'finished') continue;
                                    const epp = posFromCoord(ec);
                                    if (epp < 0) continue;
                                    const dist = (pp - epp + PL) % PL;
                                    if (dist >= 1 && dist <= 6) {
                                        // Địch đang trong tầm capture quân ta
                                        // Penalty tăng khi dist nhỏ (càng gần = càng nguy)
                                        score -= 28 - dist * 2; // dist=1→-26, dist=6→-16
                                    }
                                }
                            }
                        }
                        ...
                        // --- OPPORTUNITY BONUS (v9.1): thưởng khi ta có thể bắt địch ---
                        // Quân địch đang trong tầm bắt của ta → cơ hội tốt
                        if (!isSafePos(pp)) {
                            for (const myPP of myActivePP) {
                                const dist = (pp - myPP + PL) % PL;
                                if (dist >= 1 && dist <= 6) {
                                    score += 18; // Ta đang có thể capture quân địch này
                                }
                            }
                        }
                    }
        ...
        if (isOurTurn) {
            ...
            for (let d = 1; d <= 6; d++) {
                ...
                for (const mv of moves) {
                    ...
                    // FIX v9.1: dice=6 bonus turn → vẫn lượt ta; dice≠6 → chuyển địch
                    const val = expectiminimax(ns, depth - 1, d === 6);
            ...
            for (let d = 1; d <= 6; d++) {
                ...
                for (const mv of moves) {
                    const ns = applySearchMove(cloneSearchState(state), mv);
                    // FIX v9.1: dice=6 địch bonus turn → vẫn lượt địch; dice≠6 → trả ta
                    const val = expectiminimax(ns, depth - 1, d !== 6);
    ...
    function immScore(move, pi, dice, enemies, parsed, oppCnt, phase) {
        ...
        // --- Rule 6b: NEAR-FINISH MEGA-BONUS (v9.1 — thay thế Hard Rule 2) ---
        // Quân d2f≤6: có thể về đích chỉ bằng 1 lượt roll bất kỳ → GIÁ TRỊ RẤT CAO
        // Đảm bảo bot ưu tiên di chuyển quân gần đích hơn deploy quân mới khi dice=6
        // Thang điểm: d2f=6→+120, d2f=5→+180, d2f=4→+240, d2f=3→+300, d2f=2→+360, d2f=1→+420
        if (!pi.atHome && !move.finish && pi.d2f >= 1 && pi.d2f <= 6) {
            const megaBonus = (7 - pi.d2f) * 60; // linear tăng khi gần đích
            sc += megaBonus;
            // Extra bonus: bước này đưa quân vào HS (giá trị cực kỳ cao)
            if (move.to && HS[String(_myPos)]?.includes(move.to)) sc += 90;
        }
    ...
    function chonQuanPro(pieces, myPos, dice, parsed) {
        ...
        // ==============================================================
        // HARD RULE 2 ĐÃ XÓA (v9.1)
        // Lý do: cứng nhắc quá — deploy ngay cả khi quân đang cách đích ≤8 bước
        // (ví dụ: quân d2f=7 + dice=6 → bước tiếp theo có thể về đích, nhưng
        //  Hard Rule 2 lại deploy quân mới — SAI chiến thuật)
        // Thay thế: near-finish mega-bonus trong immScore tự nhiên ưu tiên quân
        // sắp về hơn deploy. Nếu THỰC SỰ tốt hơn để deploy → scoring quyết định.
        // ==============================================================
    ...
    // ================================================================
    function boot() {
        log('🎲 Ludo Auto-Play PRO v9.1 - Dice6 BonusTurn + ThreatAware + NearFinishBonus');

...
(function() {
    ...
    async function handleResult(d) {
        ...
        if (won) {
            ...
            // ✅ AUTO DỪNG KHI KINH (kể cả kinh trùng nhiều người cùng win)
            log('🛑 Đã kinh! Auto dừng bot...');
            S.running = false;
            if (iv) { clearInterval(iv); iv = null; }
            S.action = '🏆 Đã kinh - Bot đã dừng';
        } else {
            S.losses++;
            S.streak++;
            S.profit -= S.bet;
    ...
    async function loop() {
        ...
        try {
            ...
            if (S.status === 0) {
                ...
                if (!hasTicket) {
                    S.action = '\U0001f3ab Chu\u1ea9n b\u1ecb...';
                    calcBet();
                    await configureGame();
                    await sleep(CFG.DELAY);
                    await addBots();
                    await sleep(CFG.DELAY);
                    const ok = await startGame(); // muave
                    if (ok) {
                        // Luu state + reload de Vue chay game
                        window.name = JSON.stringify({s:S.streak, p:S.profit, w:S.wins, l:S.losses, r:S.round, b:S.bet, ib:S.initBal});
                        log('\U0001f504 Reload de Vue chay game...');
                        location.reload();
                    }
                } else {
                    S.action = '\U0001f916 Th\u00eam bot...';
            ...
            // === STATUS 1: PLAYING - Vue dang chay game, cho ket qua ===
            if (S.status === 1) {
                S.action = '\U0001f3ae Theo d\u00f5i...';

                if (d.ketquas?.userWin && S.processedRound !== 2) {
                    await handleResult(d);
                }

                if (S.processedRound === 2 && CFG.AUTO_RESTART && S.running) {
                    await continueGame();
                    // Reload de Vue refresh + bat dau vong moi
                    window.name = JSON.stringify({s:S.streak, p:S.profit, w:S.wins, l:S.losses, r:S.round, b:S.bet, ib:S.initBal});
                    log('\U0001f504 Game xong, reload cho vong moi...');
                    location.reload();
                }

                if (d.ketquas) {
                    const drawn = d.ketquas.data?.length || 0;
                    S.action = `\U0001f3af ${drawn} s\u1ed1`;
                }
                return;
            }

            // === STATUS 2: ENDED - xu ly ket qua ===
            if (S.status === 2) {
                ...
                S.action = '\U0001f3c1 K\u1ebft th\u00fac';

                if (S.processedRound !== 2) {
                    if (d.ketquas?.userWin) {
                        await handleResult(d);
                    } else {
                        S.action = '\U0001f3c1 \u0110\u1ee3i k\u1ebft qu\u1ea3...';
                    }
                }
                ...
                if (S.processedRound === 2 && CFG.AUTO_RESTART && S.running) {
                    await continueGame();
                    window.name = JSON.stringify({s:S.streak, p:S.profit, w:S.wins, l:S.losses, r:S.round, b:S.bet, ib:S.initBal});
                    log('\U0001f504 Reload cho vong moi...');
                    location.reload();
                }
                return;
            }
...
<button id="cfg-close" style="flex:1;background:#333;color:#888;border:1px solid #555;border-radius:6px;padding:8px;cursor:pointer;">Hủy</button>
    ...
    async function init() {
        ...
        if (!d) {
            ...
            // Khoi phuc state tu window.name (persist xuyen reload)
        try {
            const saved = JSON.parse(window.name || '{}');
            if (typeof saved.s === 'number') S.streak = saved.s;
            if (typeof saved.p === 'number') S.profit = saved.p;
            if (typeof saved.w === 'number') S.wins = saved.w;
            if (typeof saved.l === 'number') S.losses = saved.l;
            if (typeof saved.r === 'number') S.round = saved.r;
            if (typeof saved.b === 'number') S.bet = saved.b;
            if (typeof saved.ib === 'number') S.initBal = saved.ib;
            log('\u21a9\ufe0f Phuc hoi state: streak=' + S.streak + ' profit=' + S.profit);
        } catch(e) {}
        window.name = ''; // Xoa sau khi phuc hoi
        loadConfig();
        createUI();
            return;
        }

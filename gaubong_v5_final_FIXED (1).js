...
(function () {
    ...
    const AIJournal = (() => {
        ...
        function add(entry) {
            entries.push(Object.assign({
                ...
                pick: null, pickLabel: '', confidence: null, reason: '', approach: '',
        ...
        // ── Day la phan "AI khon dan theo thoi gian" thuc su ─────────────────
        // Thay vi chi cho AI xem 6 ban ghi gan nhat (short-term, de bi nhieu),
        // ham nay tong hop ty le trung theo TUNG KIEU LAP LUAN (approach) ma
        // chinh AI da tu gan nhan, tren mot cua so dai hon (vd 60 ban ghi).
        // Ket qua duoc dua nguoc lai vao prompt lan sau de AI tu dieu chinh
        // trong so theo bang chung thuc te — day la dieu V5 engine (rule-based
        // co dinh) khong the tu lam duoc vi no khong "suy luan theo loai".
        function approachStats(limit = 60) {
            const rows = (limit > 0 ? entries.slice(-limit) : entries)
                .filter(r => r.hit !== null && r.approach);
            const byTag = {};
            rows.forEach(r => {
                const tag = r.approach || 'other';
                byTag[tag] = byTag[tag] || { hits: 0, total: 0 };
                byTag[tag].total++;
                if (r.hit) byTag[tag].hits++;
            });
            return Object.entries(byTag)
                .map(([tag, v]) => ({
                    ...
                    total: v.total,
                    hits: v.hits,
                    accuracy: v.total ? +(v.hits / v.total).toFixed(3) : 0,
                    reliable: v.total >= 8, // duoi 8 mau thi chua du tin cay
                }))
        ...
        return { add, recordOutcome, getRecent, stats, approachStats, wipe };
    })();
    ...
    const V5 = (() => {
        ...
        // ── Per-animal win-rate stats (lifetime vs recent) ───────────────────
        // AI cần so sánh hai chiều này để phát hiện hot-streak vs mean-reversion.
        // deviation > 0 = con đang hot (cẩn thận), deviation < 0 = cold (cơ hội revert).
        function computeAnimalStats(n) {
            if (!history.length || !n) return [];
            const allWins = Array(n).fill(0);
            const r20Wins = Array(n).fill(0);
            const recent20 = history.slice(-20);
            history.forEach(r => { if (r.winner >= 0 && r.winner < n) allWins[r.winner]++; });
            recent20.forEach(r => { if (r.winner >= 0 && r.winner < n) r20Wins[r.winner]++; });
            const total = history.length;
            const r20Total = recent20.length;
            const expected = 1 / n; // fair baseline = 14.3% cho 7 con
            return Array.from({ length: n }, (_, i) => {
                const lifeWR = total ? allWins[i] / total : 0;
                const recWR  = r20Total ? r20Wins[i] / r20Total : 0;
                return {
                    id      : i + 1,
                    lifeWins: allWins[i],
                    lifeWR  : +lifeWR.toFixed(3),
                    recWins : r20Wins[i],   // trong 20 vòng gần nhất
                    recWR   : +recWR.toFixed(3),
                    // deviation: dương = đang hot (nguy cơ revert), âm = cold (cơ hội)
                    dev     : +(recWR - expected).toFixed(3),
                };
            });
        }

        // ── Transition matrix: sau con X thắng → con nào thắng tiếp? ────────
        // Đây là "line-up pattern" quan trọng nhất mà AI thiếu hiện tại.
        function computeTransitionMatrix(n) {
            if (history.length < 5 || !n) return null;
            const lastWinner = history[history.length - 1]?.winner;
            if (lastWinner == null || lastWinner < 0 || lastWinner >= n) return null;

            // Đếm toàn bộ history (không chỉ recent) để có sample size đủ lớn
            const counts = Array(n).fill(0);
            let totalFollowing = 0;
            for (let i = 0; i < history.length - 1; i++) {
                if (history[i].winner === lastWinner) {
                    const next = history[i + 1].winner;
                    if (next >= 0 && next < n) { counts[next]++; totalFollowing++; }
                }
            }
            if (!totalFollowing) return null;

            const nextProbs = counts
                .map((cnt, id) => ({
                    id  : id + 1,
                    prob: +(cnt / totalFollowing).toFixed(3),
                    cnt,
                }))
                .sort((a, b) => b.prob - a.prob)
                .slice(0, 4); // top 4 ứng viên

            return {
                lastWinner: lastWinner + 1,
                sampleSize: totalFollowing,          // bao nhiêu lần con này thắng trong lịch sử
                nextProbs,                           // top 4 con có xác suất thắng tiếp theo cao nhất
                reliable  : totalFollowing >= 12,   // chỉ tin cậy khi có >= 12 mẫu
            };
        }

        // ── Position stats: dùng TOÀN BỘ thứ hạng 1-7, không chỉ ai thắng ────
        // Một con về nhì/ba liên tục (avgPos thấp) là tín hiệu mạnh hơn nhiều
        // so với chỉ đếm số lần về nhất — đây là dữ liệu mà trước giờ AI
        // hoàn toàn không được thấy dù engine vẫn lưu đủ trong history.
        function computePositionStats(n) {
            if (!history.length || !n) return [];
            const lifeSum = Array(n).fill(0);
            const lifeCnt = Array(n).fill(0);
            const recSum  = Array(n).fill(0);
            const recCnt  = Array(n).fill(0);
            const recent20 = history.slice(-20);

            history.forEach(r => {
                if (!Array.isArray(r.order)) return;
                r.order.forEach((id, pos) => {
                    if (id >= 0 && id < n) { lifeSum[id] += pos; lifeCnt[id]++; }
                });
            });
            recent20.forEach(r => {
                if (!Array.isArray(r.order)) return;
                r.order.forEach((id, pos) => {
                    if (id >= 0 && id < n) { recSum[id] += pos; recCnt[id]++; }
                });
            });

            const mid = (n - 1) / 2; // vị trí trung bình kỳ vọng nếu hoàn toàn ngẫu nhiên
            return Array.from({ length: n }, (_, i) => {
                const lifeAvgPos = lifeCnt[i] ? lifeSum[i] / lifeCnt[i] : mid;
                const recAvgPos  = recCnt[i] ? recSum[i] / recCnt[i] : mid;
                return {
                    id        : i + 1,
                    lifeAvgPos: +lifeAvgPos.toFixed(2), // 0 = luon ve nhat, n-1 = luon ve bet
                    recAvgPos : +recAvgPos.toFixed(2),
                    // posEdge < 0 = đang đua tốt hơn mức trung bình kỳ vọng (về cao hơn)
                    posEdge   : +((mid - lifeAvgPos)).toFixed(2),
                };
            }).sort((a, b) => a.lifeAvgPos - b.lifeAvgPos); // tốt nhất lên đầu
        }

        // ── Head-to-head (Bradley-Terry kiểu thực nghiệm): giữa 2 con bất kỳ, ──
        // con nào thường về trước con kia hơn? Chỉ giữ lại các cặp có chênh
        // lệch rõ rệt (>=58% hoặc <=42%) và đủ mẫu để tránh nhiễu nhỏ lẻ.
        function computeHeadToHead(n, topK = 6) {
            if (!history.length || n < 2) return [];
            const aheadCount = {}; // key `${a}_${b}` = số lần a về trước b
            const total = {};

            history.forEach(r => {
                if (!Array.isArray(r.order) || r.order.length < n) return;
                const pos = posMap(r.order);
                for (let a = 0; a < n; a++) {
                    for (let b = a + 1; b < n; b++) {
                        if (pos[a] == null || pos[b] == null) continue;
                        const key = `${a}_${b}`;
                        total[key] = (total[key] || 0) + 1;
                        if (pos[a] < pos[b]) aheadCount[key] = (aheadCount[key] || 0) + 1;
                    }
                }
            });
            ...
            Object.keys(total).forEach(key => {
                ...
                const t = total[key];
                if (t < 15) return; // mau qua nho, bo qua de tranh nhieu
                const aWinProb = (aheadCount[key] || 0) / t;
                if (aWinProb >= 0.58 || aWinProb <= 0.42) {
                    pairs.push({
                        a: a + 1, b: b + 1,
                        leader: aWinProb >= 0.5 ? a + 1 : b + 1,
                        prob: +(aWinProb >= 0.5 ? aWinProb : 1 - aWinProb).toFixed(3),
                        sample: t,
                    });
                }
            });

            return pairs.sort((x, y) => y.prob - x.prob || y.sample - x.sample).slice(0, topK);
        }

        function getMemorySnapshot(limit = C.MEMORY_WINDOW) {
            // Infer n từ history gần nhất (fallback = 7)
            const nAnimals = (() => {
                for (let i = history.length - 1; i >= 0; i--) {
                    if (Array.isArray(history[i].order) && history[i].order.length > 0)
                        return history[i].order.length;
                }
                return 7;
            })();
            ...
            // ── Sắp xếp signals ưu tiên high LIFETIME support (không phải recent) ──
            // Lý do: nếu chỉ sort theo confidence (blends recentWR), con đang hot
            // luôn nổi lên đầu và AI bị dắt mũi theo hot-streak.
            const topSignals = strategies
                .slice()
                .sort((a, b) => {
                    const aSupport = a.wins + a.losses;
                    const bSupport = b.wins + b.losses;
                    // Ưu tiên: support cao + confidence cao (long-term edge, không phải streak)
                    return (b.confidence * Math.log1p(bSupport)) - (a.confidence * Math.log1p(aSupport)) || a.id - b.id;
                })
                .slice(0, 6)
                .map(s => ({
                    id: s.id,
            ...
            // ── Ba data quan trọng: thống kê thắng, ma trận chuyển tiếp, ────────
            // và 2 cái MỚI dùng full ranking 1-7: position stats + head-to-head
            const animalStats        = computeAnimalStats(nAnimals);
            const transitionFromLast = computeTransitionMatrix(nAnimals);
            const positionStats      = computePositionStats(nAnimals);
            const headToHead         = computeHeadToHead(nAnimals);
            ...
            return {
                ...
                recentRounds   : recent,
                topSignals,
                // ── Mới: thống kê định lượng từng con ──────────────────────────
                // dev > +0.05 = đang hot-streak (cảnh báo revert)
                // dev < -0.05 = đang cold (cơ hội mean-reversion)
                animalStats,
                // ── Mới: sau con vừa thắng → ai có xu hướng thắng tiếp? ────────
                transitionFromLast,
                // ── Mới: dùng FULL thứ hạng 1-7, không chỉ ai về nhất ──────────
                // avgPos thấp = con này đua ổn định tốt dù không phải lúc nào cũng thắng
                positionStats,
                // ── Mới: cặp con nào thường về trước con kia (Bradley-Terry) ──
                headToHead,
                // ── Guidance đúng: mean-reversion + transition, không phải trend-follow ──
                guidance: auditAll.total >= C.MIN_HISTORY
                    ? 'Apply mean-reversion logic: high deviation (dev > +0.10) animals are overdue to regress. Use transitionFromLast as primary signal when sampleSize >= 12. Do NOT chase hot streaks — recency alone is not statistical edge.'
                    : 'Warmup: use transition probabilities and lifetime win rates as primary signals. Ignore short-term streaks with fewer than 5 samples.',
            };
        }
    ...
    const AI = (() => {
        ...
        const VALID_APPROACHES = ['transition', 'reversion', 'position', 'pattern', 'other'];
        ...
        function normalizePick(text, n) {
            ...
            if (json && typeof json === 'object') {
                ...
                approach = String(json.approach ?? json.tag ?? json.method ?? '').toLowerCase().trim();
            }
            if (!VALID_APPROACHES.includes(approach)) approach = 'other';
        ...
        function buildPrompt({ n, options, memory, journal, perf }) {
            const expectedWR = (1 / n).toFixed(3);
            const sections   = [];

            // ── ROLE & MISSION ────────────────────────────────────────────────────
            sections.push(
                `You are a statistical analyst for a ${n}-animal virtual racing game (${n} animals, 1 winner per round, FULL 1-to-${n} finishing order recorded every round).\n` +
                `Fair baseline win rate per animal = ${expectedWR} (1/${n}).\n` +
                `Your goal: identify the single pick with the best LONG-TERM statistical edge — NOT short-term momentum.\n` +
                `Return ONLY valid JSON. No markdown. Schema: {"pick":1,"confidence":0.0,"reason":"brief","approach":"transition|reversion|position|pattern|other"}\n` +
                `Rules: pick = integer 1–${n}. confidence = 0.0–1.0. approach = the PRIMARY signal type your pick relied on (see framework below — tag honestly, this is tracked and fed back to you).`
            );

            // ── ANALYTICAL FRAMEWORK (bắt buộc AI suy luận theo thứ tự, gắn tag) ─
            sections.push(
                `DECISION FRAMEWORK — evaluate all 4 signal types below, then pick the strongest one. Tag your final "approach" accordingly:\n` +
                `[transition] After the last winner, which animal historically wins next most often?\n` +
                `   Use "transitionFromLast.nextProbs" — strong if prob > ${(1/n+0.05).toFixed(2)} AND sampleSize >= 12.\n` +
                `[reversion] Mean-reversion on win-rate: check animalStats[].dev (deviation from baseline ${expectedWR}).\n` +
                `   dev > +0.10 → HOT, likely to regress → avoid. dev < -0.08 → COLD, due for reversion → consider.\n` +
                `[position] Full-ranking signal (NOT just wins): check positionStats[].lifeAvgPos (0=always 1st, ${n-1}=always last)\n` +
                `   and headToHead pairs — an animal with consistently low avgPos or a strong head-to-head lead over a co-favorite\n` +
                `   is a more reliable pick than one that just won recently by luck.\n` +
                `[pattern] Pattern-engine signals in topSignals — only trust support >= 10 and !disabled.\n` +
                `\nSTRICTLY AVOID:\n` +
                `- Picking the animal with highest recentWR alone (that is trend-following, not statistics).\n` +
                `- Picking any animal that has won 3+ of the last 5 rounds (hot streak = regression risk) UNLESS positionStats/headToHead independently confirm it is a genuinely strong performer, not just lucky.\n` +
                `- Tagging "other" when one of the 4 signal types above clearly applies — only use "other" if signals conflict with no clear winner.`
            );

            // ── PERFORMANCE FEEDBACK (đây là phần "AI khôn dần theo thời gian") ─
            // Khac voi cac section khac (du lieu round), day la du lieu VE CHINH
            // AI: kieu lap luan nao tu truoc den nay thuc su an khop voi ket qua
            // thuc te. Day la vong lap thich nghi that su, khong phai static rule.
            if (Array.isArray(perf) && perf.length) {
                sections.push(
                    `YOUR PERFORMANCE TRACK RECORD by approach (last up to 60 graded rounds — this is YOUR real accuracy, use it to reweight):\n` +
                    JSON.stringify(perf) +
                    `\nIf an approach shows reliable:true and accuracy is clearly higher than others, lean toward it when signals are ambiguous. ` +
                    `If an approach shows reliable:false (total < 8), do not over-trust it yet — treat its accuracy as noisy.`
                );
            }

            // ── PER-ANIMAL STATISTICS (win-based) ───────────────────────────────
            if (Array.isArray(memory?.animalStats) && memory.animalStats.length) {
                sections.push(
                    `PER-ANIMAL WIN STATS (lifeWR = lifetime win rate, recWR = last 20 rounds, dev = recWR − ${expectedWR}):\n` +
                    `Positive dev = hot-streak risk. Negative dev = cold / mean-reversion candidate.\n` +
                    JSON.stringify(memory.animalStats)
                );
            }

            // ── POSITION STATS (full ranking, không chỉ thắng) ──────────────────
            if (Array.isArray(memory?.positionStats) && memory.positionStats.length) {
                sections.push(
                    `PER-ANIMAL POSITION STATS (uses the FULL 1-to-${n} finishing order, not just wins):\n` +
                    `lifeAvgPos: average finishing position over all rounds (0=always 1st, ${n-1}=always last).\n` +
                    `recAvgPos: same but last 20 rounds only. Sorted best-to-worst by lifeAvgPos.\n` +
                    JSON.stringify(memory.positionStats)
                );
            }

            // ── HEAD-TO-HEAD ──────────────────────────────────────────────────────
            if (Array.isArray(memory?.headToHead) && memory.headToHead.length) {
                sections.push(
                    `HEAD-TO-HEAD (pairwise, only pairs with clear lean and sample >= 15):\n` +
                    `"leader" finishes ahead of the other animal "prob" fraction of the time when both race.\n` +
                    JSON.stringify(memory.headToHead)
                );
            }

            // ── TRANSITION MATRIX ─────────────────────────────────────────────────
            if (memory?.transitionFromLast) {
                const tr = memory.transitionFromLast;
                const reliableNote = tr.reliable ? '(RELIABLE — strong candidate signal)' : '(low sample, secondary only)';
                sections.push(
                    `TRANSITION FROM LAST WINNER #${tr.lastWinner} — ${tr.sampleSize} historical samples ${reliableNote}:\n` +
                    JSON.stringify(tr.nextProbs)
                );
            }

            // ── RECENT ROUNDS (context, không phải signal chính) ─────────────────
            if (Array.isArray(memory?.recentRounds) && memory.recentRounds.length) {
                const last5 = memory.recentRounds.slice(-5).map(r => `R${r.round}: winner=#${r.winner}`).join(', ');
                sections.push(
                    `RECENT CONTEXT (last ${Math.min(5, memory.recentRounds.length)} rounds — for context only, DO NOT chase these):\n${last5}`
                );
            }

            // ── PATTERN ENGINE SIGNALS ────────────────────────────────────────────
            if (Array.isArray(memory?.topSignals) && memory.topSignals.length) {
                const validSignals = memory.topSignals.filter(s => !s.disabled && s.support >= 10);
                if (validSignals.length) {
                    sections.push(
                        `PATTERN ENGINE (long-term signals, sorted by support × confidence):\n` +
                        JSON.stringify(validSignals.map(s => ({
                            predict: s.predict, class: s.class,
                            confidence: s.confidence, support: s.support,
                        })))
                    );
                }
            }
            ...
            if (memory?.totalRounds) {
                sections.push(
                    `SYSTEM STATS: ${memory.totalRounds} rounds recorded | ` +
                    `lifetime accuracy: ${((memory.lifetimeAccuracy || 0) * 100).toFixed(1)}% | ` +
                    `recent accuracy: ${((memory.recentAccuracy || 0) * 100).toFixed(1)}%`
                );
            }
            if (memory?.guidance) {
                sections.push(`ANALYTICAL GUIDANCE: ${memory.guidance}`);
            }

            // ── JOURNAL (self-review, short window for tone/context only) ───────
            if (Array.isArray(journal) && journal.length) {
                const recentJ = journal.slice(-6).map(j => ({
                    picked  : j.pickLabel || j.pick,
                    approach: j.approach || '',
                    actual  : j.actualLabel || j.actual,
                    correct : j.hit,
                    reason  : (j.reason || '').slice(0, 60),
                }));
                sections.push(
                    `YOUR LAST 6 PICKS (short-term context — for the bigger picture use PERFORMANCE TRACK RECORD above, not this):\n` +
                    JSON.stringify(recentJ)
                );
            }
        ...
        async function callOpenAI({ key, model, baseUrl, prompt }) {
            const payload = {
                model,
                messages: [
                    { role: 'system', content: 'You are a rigorous statistical analyst. Apply mean-reversion and transition-probability reasoning. Do not chase recent hot streaks — that is a cognitive bias, not statistical edge. Return JSON only.' },
        ...
        async function callAnthropic({ key, model, baseUrl, prompt, authMode }) {
            const payload = {
                ...
                max_tokens: 160,
                temperature: 0,
                system: 'You are a rigorous statistical analyst. Apply mean-reversion and transition-probability reasoning. Do not chase recent hot streaks — that is a cognitive bias, not statistical edge. Return JSON only.',
        ...
        async function choose({ n, options, memory, round }) {
            ...
            try {
                ...
                // ── Feedback loop: ty le trung theo TUNG kieu lap luan, tinh tren
                // cua so dai han (60 ban ghi) — day la phan giup AI "khon dan"
                // thuc su thay vi chi lap lai cung mot bo huong dan tinh.
                const perf = AIJournal.approachStats(60);
                const prompt = buildPrompt({ n, options, memory, journal: journalCtx, perf });
    ...
    const Panel = (() => {
        ...
        function updateDebug(debug, opts) {
            ...
            // ── Innovation: hien thi truc quan animalStats + transition signal ──
            // Giup nguoi dung KIEM CHUNG AI co dang theo dung mean-reversion logic
            // hay khong, thay vi tin mu vao JSON. Con nao dev > +0.10 se highlight
            // do (canh bao hot-streak), dev < -0.08 highlight xanh (co hoi).
            let statsStrip = '';
            if (Array.isArray(mem.animalStats) && mem.animalStats.length) {
                const chips = mem.animalStats.map(s => {
                    const cls = s.dev > 0.10 ? 'hot' : s.dev < -0.08 ? 'cold' : 'neutral';
                    const sign = s.dev >= 0 ? '+' : '';
                    return `<span class="dta-achip ${cls}" title="lifeWR ${pct(s.lifeWR)}">#${s.id} ${sign}${(s.dev * 100).toFixed(0)}%</span>`;
                }).join('');
                statsStrip += `<div class="dta-astats"><span class="dta-astats-label">Dev vs ${pct(1 / mem.animalStats.length)}</span>${chips}</div>`;
            }
            if (mem.transitionFromLast) {
                const tr = mem.transitionFromLast;
                const top = tr.nextProbs?.[0];
                if (top) {
                    const relCls = tr.reliable ? 'reliable' : 'weak';
                    statsStrip += `<div class="dta-transition ${relCls}">
                        Sau #${tr.lastWinner} → #${top.id} (${pct(top.prob)}, n=${tr.sampleSize})${tr.reliable ? '' : ' · mau nho'}
                    </div>`;
                }
            }
        ...
        const APPROACH_LABEL = {
            transition: 'CHUYEN TIEP', reversion: 'HOI QUY TB', position: 'XEP HANG',
            pattern: 'PATTERN', other: 'KHAC',
        };
        ...
        function updateAIAnalysis(cur, journalRecent) {
            ...
            const approachBadge = cur?.approach
                ? `<span class="aia-approach ${escapeHtml(cur.approach)}">${APPROACH_LABEL[cur.approach] || cur.approach.toUpperCase()}</span>`
                : '';
            card.innerHTML = `
                <div class="aia-top">
...
.aia-approach { font-size:8px; font-weight:800; letter-spacing:.3px; padding:1px 5px; border-radius:4px; color:#9fb4ff; background:#161a2c; }
.aia-approach.transition { color:#ffd36a; background:#241f0a; }
.aia-approach.reversion { color:#48d18a; background:#0d2518; }
.aia-approach.position { color:#7ec8ff; background:#0a1f2e; }
.aia-approach.pattern { color:#c79bff; background:#1c0f2e; }
.aia-approach.other { color:#8791a3; background:#171a21; }
...
.dta-astats { display:flex; flex-wrap:wrap; gap:3px; align-items:center; margin:0 0 5px; }
.dta-astats-label { font-size:9px; color:#596273; margin-right:2px; }
.dta-achip { font-size:9px; font-weight:700; padding:2px 5px; border-radius:8px; background:#151a23; color:#8791a3; }
.dta-achip.hot { background:#3a1a1a; color:#ff7a7a; }
.dta-achip.cold { background:#143226; color:#48d18a; }
.dta-transition { font-size:10px; color:#ffd36a; background:#1c1a12; border-radius:5px; padding:4px 6px; margin:0 0 6px; }
.dta-transition.weak { color:#8791a3; background:#11151d; }
